...
table(colData(pancreasMuraro)$label); dim(pancreasMuraro)
...
rownames(pancreasMuraro) <- unlist(lapply(strsplit(rownames(pancreasMuraro), split="__"), function(x)x[[1]]))
...
table(colData(pancreasBaron)$label); dim(pancreasBaron)
...
```{r, out.width="1800px", out.height="360px" }
...
similarityTable5TopFeatures <- clusterFoldSimilarity(scList=singlecellObjectList, topNFeatures=5, nSubsampling=24)
...
```{r, out.width="1800px", out.height="360px" }
similarityTableAllValues <- clusterFoldSimilarity(scList=singlecellObjectList, sampleNames = c("human", "humanNA"), topN=Inf)
...
table(colData(pancreasBaronMM)$label); dim(pancreasBaronMM)
